...
from MitsuriRobot.modules.helper_funcs.string_handling import markdown_parser

import MitsuriRobot.modules.sql.feds_sql as sql

from MitsuriRobot.modules.helper_funcs.alternate import (
    send_message,
    typing_action,
    send_action,
)
from MitsuriRobot.modules.helper_funcs.decorators import Asukacmd, Asukacallback
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Asuka_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Asuka_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Asuka_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Asuka_fbanned_users.csv",
